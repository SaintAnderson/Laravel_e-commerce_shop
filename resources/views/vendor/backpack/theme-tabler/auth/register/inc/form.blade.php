...
<form role="form" method="POST" action="{{ route('backpack.auth.register') }}">
    @csrf
    <div class="mb-3">
        <label class="form-label" for="first_name">{{ trans('backpack::base.first_name') }}</label>
    ...
    <div class="mb-3">
        <label class="form-label" for="last_name">{{ trans('backpack::base.last_name') }}</label>
        <input autofocus tabindex="1" type="text" class="form-control {{ $errors->has('last_name') ? 'is-invalid' : '' }}" name="last_name" id="last_name" value="{{ old('last_name') }}">
        @if ($errors->has('last_name'))
            <div class="invalid-feedback">{{ $errors->first('last_name') }}</div>
        @endif
    </div>
    <div class="mb-3">
        <label class="form-label" for="middle_name">{{ trans('backpack::base.middle_name') }} (Необязательно)</label>
        <input autofocus tabindex="1" type="text" class="form-control {{ $errors->has('middle_name') ? 'is-invalid' : '' }}" name="middle_name" id="middle_name" value="{{ old('middle_name') }}">
