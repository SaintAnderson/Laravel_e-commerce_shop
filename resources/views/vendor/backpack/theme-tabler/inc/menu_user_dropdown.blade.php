    ...
    <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown" aria-label="Open user menu">
        <span class="avatar avatar-sm rounded-circle">
            <img class="avatar avatar-sm rounded-circle bg-transparent" src="{{ backpack_avatar_url(backpack_auth()->user()) }}"
                alt="{{ backpack_user()->first_name }} {{ backpack_user()->last_name }} {{ backpack_user()->middle_name }}" onerror="this.style.display='none'"
        ...
        <div class="d-none d-xl-block ps-2">
            <div>{{ backpack_user()->first_name }} {{ backpack_user()->last_name }} {{ backpack_user()->middle_name }}</div>
